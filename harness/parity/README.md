...
**launcher** (`run.sh`) replays each through the PHP bridge, the Go bridge and the
Node bridge and requires the **same issue and the same message** from all three:
...
either `parity: all fixtures, queries, writes and transactions agree on all three
...
fixture, query, write and transaction through PHP, Go and Node, checks that the
neighbouring residue (`.start.yaml.memless-tmp`) is **byte-for-byte intact** after a
successful load (decision D16), and sources `queries.sh`, `writes.sh` and
`transactions.sh` for the three tabular parts. A `!disk` line forces the atomic
