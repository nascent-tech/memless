...
use memless_engine::{load, read, reload, Instance};
...
fn main() {
    println!("k\tload_us_p50\tload_us_p95\treload_us_p50\treload_us_p95\tsuite100_us_p50\tsuite100_us_p95");
    SIZES.iter().for_each(|size| println!("{}", report(*size)));
}

fn report(size: usize) -> String {
    let (load50, load95) = sample(size, time_load);
    let (reload50, reload95) = sample(size, time_reload);
    let (suite50, suite95) = sample(size, time_suite);
    format!("{size}\t{load50}\t{load95}\t{reload50}\t{reload95}\t{suite50}\t{suite95}")
}
...
fn time_reload(_size: usize, text: &str) -> Duration {
    let (dir, mut instance) = fresh(text);
    let start = Instant::now();
    reload(read, &mut instance).expect("reload");
    let elapsed = start.elapsed();
    drop(instance);
    let _ = std::fs::remove_dir_all(&dir);
    elapsed
...
fn time_suite(_size: usize, text: &str) -> Duration {
    let (dir, mut instance) = fresh(text);
    let start = Instant::now();
    (0..SUITE).for_each(|_| reload(read, &mut instance).expect("suite reload"));
    let elapsed = start.elapsed();
    drop(instance);
    let _ = std::fs::remove_dir_all(&dir);
    elapsed
...
fn seed(text: &str) -> (String, std::path::PathBuf) {
    let dir = std::env::temp_dir().join(format!("memless-bench-reload-{}", unique()));
