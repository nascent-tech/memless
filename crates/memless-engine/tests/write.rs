...
use memless_engine::{load, parse, read, replace_file, render, write, Instance, Refusal};
...
fn an_accepted_write_rewrites_the_file_byte_for_byte() {
    ...
    let affected = write(parse, replace_file, &mut inst, "INSERT INTO logs (id, line) VALUES (2, 'b')").unwrap();
...
fn a_write_without_a_change_touches_no_disk() {
    ...
    let affected = write(parse, boom, &mut inst, "UPDATE logs SET line = 'a' WHERE id = 1").unwrap();
    assert_eq!(affected, 1);
}
...
fn a_disk_failure_leaves_memory_behind() {
    ...
    let refusal = write(parse, boom, &mut inst, "UPDATE logs SET line = 'z' WHERE id = 1").unwrap_err();
...
fn a_select_passed_to_execute_is_refused() {
    ...
    let refusal = write(parse, replace_file, &mut inst, "SELECT * FROM logs").unwrap_err();
